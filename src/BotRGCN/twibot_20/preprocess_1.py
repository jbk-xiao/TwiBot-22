...
print('loading rawtorch; print data')
...
tweet_dict = {i: [] for i in range(len(user))}
for i in tqdm(range(len(user))):
    try:
        ...
        tweet_dict[edge['source_id'][i]].append(tweet['text'][edge['target_id'][i] + len(user)])

np.save('processed_data/each_user_tweets.npy', tweet_dict)
